...
export const getBoilsListInputSchema = z.object({
  ...
  problem: z.array(z.string()).nullish(),
  page: z.number().min(1).default(1),
  limit: z.number().min(1).max(100).default(10),
});

export const getBoilsStatsInputSchema = z.object({
  startDate: z.coerce.date().default(() => {
    const now = new Date();
    return new Date(now.getFullYear(), now.getMonth(), 1);
  }),
...
export const getBoilDetailSchema = z.object({
  boilId: z.number(),
});

export type TGetBoilsListInput = z.infer<typeof getBoilsListInputSchema>;
export type TGetBoilsStatsInput = z.infer<typeof getBoilsStatsInputSchema>;
export type TGetBoilDetailInput = z.infer<typeof getBoilDetailSchema>;
